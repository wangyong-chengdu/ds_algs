...
## 解法2
- 思路：二分查找，满足两个条件：
    - 分割线左边和右边元素个数相等，或左右元素个数比右边元素个数多1.
    - 分隔线左边所有元素小于分割线右边所有元素。（很重要，并非元素之和而是每个元素都要，元素数组要交叉小于等于）
- 时间复杂度：o(log(m + n))
...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while (left <= right) {
            ...
            // 这样计算i,j可满足第一个条件：分割线左边和右边元素个数相等，或左右元素个数比右边元素个数多1.
            int i = (left + right) / 2;
            int j = (m + n + 1) / 2 - i;
            ...
            // 满足条件2：分隔线左边所有元素小于分割线右边所有元素。（很重要，并非元素之和而是每个元素都要，元素数组要交叉小于等于）
            // 等价于满足交叉情况：前一部分的最大值 <= 后一部分的最小值
            if (nums_im1 <= nums_j) {
                ansi = i;
