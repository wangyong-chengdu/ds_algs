package cd.wangyong.da_algs.leetcode.two_pointer.q_minimum_size_subarray_sum;
...
public class Solution {
    ...
    public int minSubArrayLen2(int s, int[] nums) {
        // build an accumulated sum array of nums, which is monotone incresing that helps for binary search
    ...
    private int findWindowStart(int[] sums, int[] nums, int i, int target) {
        int low = 0, high = i, sum = sums[i];
        while (low <= high) {
            if (sum <= target) {
                high = (low + high) / 2 - 1;
            }
